...
class MPPI_Controller_Soft:
    ...
    def __init__(self,
        ...
        # 障碍松弛变量 控制序列
        # self.ob_U = np.zeros((self.num_timesteps, self.ob_num_max),
        #                        dtype=np.float32)
        self.ob_u = [] #
        ...
        # 如果 kinematics 只有运动学代码，直接加载代码。
        # 如果 kinematics 包含动态参数获取逻辑，同时加载代码和动态参数。
        ...
        # Save the dynamics code and dynamics param updater if there is one to save.
        if (type(ob_kinematics) is list or type(ob_kinematics) is tuple):
            ob_kinematics_code = ob_kinematics[0]
            self.ob_kinematics_param_getter = ob_kinematics[1]
            self.ob_kinematics_arrs = self.ob_kinematics_param_getter()
        else:
            ob_kinematics_code = ob_kinematics
            self.ob_kinematics_param_getter = None
            self.ob_kinematics_arrs = {}

        # Save the costs code and cost param updater
        if (type(ob_state_costs) is list or type(ob_state_costs) is tuple):
            ob_costs_code = ob_state_costs[0]
            self.ob_costs_param_getter = ob_state_costs[1]
            self.ob_costs_arrs = self.ob_costs_param_getter()
        else:
            ob_costs_code = ob_state_costs
            self.ob_costs_param_getter = None
            self.ob_costs_arrs = {}        
    ...
    def get_cuda_functions(self, cuda_policy, kinematics, state_costs, ob_kinematics, ob_state_costs):
        """
        ...
        ) + cuda_policy + kinematics + state_costs + ob_kinematics + ob_state_costs+ self.cuda_rollouts()
        ...
        U_d = mod.get_global("U_d")[0] 
             ...
             # 障碍物软约束 松弛变量 控制信号的GPU地址。
        ob_U_d = mod.get_global("ob_U_d")[0] 
        ...
        ob_kinematics_adrs = {} # 松弛变量 运动学 的GPU地址
        for arr_name in self.ob_kinematics_arrs:
            ob_kinematics_adrs[arr_name] = mod.get_global(arr_name)[0]

        ob_costs_adrs = {} # 松弛变量 成本参数 的GPU地址
        for arr_name in self.ob_costs_arrs:
            ob_costs_adrs[arr_name] = mod.get_global(arr_name)[0]

            # 以便在CUDA核中访问这些参数
        return func, U_d, ob_U_d, policy_adrs, kinematics_adrs, costs_adrs, ob_kinematics_adrs, ob_costs_adrs
    ...
    def params_to_cuda(self, policy_params_adrs, kinematics_params_adrs,
                       costs_params_adrs, ob_kinematics_params_adrs,
                       ob_costs_params_adrs):
        ...
        # Transfer the ob_kinematics parameters to CUDA constant memory
        if (self.ob_kinematics_param_getter is not None):
            ob_kinematics_params = self.ob_kinematics_param_getter()
            for key in ob_kinematics_params_adrs:
                gpu_arr = self.on_gpu(ob_kinematics_params[key])
                cuda.memcpy_dtod(ob_kinematics_params_adrs[key], gpu_arr.ptr,
                                 gpu_arr.nbytes)

        # Transfer the ob_cost parameters to CUDA constant memory
        if (self.ob_costs_param_getter is not None):
            ob_costs_params = self.ob_costs_param_getter()
            for key in ob_costs_params_adrs:
                gpu_arr = self.on_gpu(ob_costs_params[key])
                cuda.memcpy_dtod(ob_costs_params_adrs[key], gpu_arr.ptr,
                                 gpu_arr.nbytes)
    ...
    def rollouts(self, state, std_n, R, weights, targets, 
                    ob, ob_num, ob_id, 
                        r_sla, std_n_sla, ob_U, ob_state_sla, cost_baseline=None):
        """
        ...
        # 松弛变量随机数
        if ob_num != 0:
            du_ob_d = self.generator.gen_normal(
                self.num_samples * self.num_timesteps * ob_num,
                np.float32) 
            # print(du_ob_d[:10])
            # print(f"NUM_du_ob_d:{int(len(du_ob_d)/(self.num_samples * self.num_timesteps))}")      
            self.du_ob = du_ob_d.get()    
            # 松弛变量随机扰动
            du_ob_d = self.on_gpu(self.du_ob, dtype=np.float32)

            # print(f"du_ob_d:{du_ob_d}")
            # 松弛变量变化率
            U_sla_cur = []
            for id in  ob_id:
                U_sla_cur.append(ob_U[id])       
            ob_U_d = self.on_gpu(ob_U)
            # 松弛变量状态
            state_sla_cur = []
            for id in  ob_id:
                state_sla_cur.append(ob_state_sla[id])
            # print(f"state_sla_cur:{state_sla_cur}")
            ob_state_sla_d = self.on_gpu(np.array(ob_state_sla))
            # 松弛变量方差
            std_n_sla = np.array([std_n_sla] * ob_num)
            # print(f"std_n_sla:{std_n_sla}")
            std_n_slad = self.on_gpu(np.array(std_n_sla))
            # 松弛变量控制率
            r_sla = np.array([r_sla] * ob_num)
            r_slad = self.on_gpu(np.array(r_sla))
            # 障碍物数量
            ob = np.array(ob)
            ob_num = np.array(ob_num)
            ob_id =  np.array(ob_id)
            # print(f"ob:{ob}")
            # print(f"ob_num:{ob_num}")
            print(f"ob_id:{ob_id}")
            ob_d = self.on_gpu(ob)# 障碍物
            ob_num_d = self.on_gpu(ob_num)
            ob_id_d = self.on_gpu(ob_id)
        else:
            ob_U_d = self.on_gpu(np.zeros(1, dtype=np.float32))
            du_ob_d = self.on_gpu(np.zeros(1, dtype=np.float32))
            ob_state_sla_d = self.on_gpu(np.zeros(1, dtype=np.float32))
            r_slad =  self.on_gpu(np.zeros(1, dtype=np.float32))
            std_n_slad = self.on_gpu(np.zeros(1, dtype=np.float32))
            ob_d = self.on_gpu(np.zeros(1, dtype=np.float32))
            ob_num_d = self.on_gpu(np.zeros(1, dtype=np.float32))
            ob_id_d= self.on_gpu(np.zeros(1, dtype=np.float32))           
        # print(f"self.U := {self.U}")
        U_d = self.on_gpu(self.U)
        weights_d = self.on_gpu(weights)
        targets_d = self.on_gpu(targets)
        # 卸载 CUDA 函数和地址
        rollout_kernel, U_d_adr, ob_U_d_adr, policy_params_adrs, kinematics_params_adrs, costs_params_adrs, ob_kinematics_params_adrs, ob_costs_params_adrs = self.cuda_functions

        # 将当前控制序列传输到 CUDA 常量内存
        cuda.memcpy_dtod(U_d_adr, U_d.ptr, U_d.nbytes)
        cuda.memcpy_dtod(ob_U_d_adr, ob_U_d.ptr, ob_U_d.nbytes)
        ...
        costs /= normalizer # costs 是一个向量，表示每条轨迹的权重，维度为：样本数
        ...
        # 操作后的结果：
        # 相乘后：
        # control_variations 的维度保持不变。
        # 每个轨迹的控制扰动已经被对应的权重 𝑤𝑖 调整过
        control_variations *= costs # control_variations 是一个矩阵，其维度为：(控制维度×时间步数)×样本数
        ...
        if ob_num != 0:
            # 从GPU中检索生成的控制扰动 du_d，并重新reshape为形状 (控制维度×时间步×样本数)
            ob_control_variations = du_ob_d.get().reshape(  # 
                (self.num_samples, self.num_timesteps * ob_num)).T # 生成的控制信号扰动。   
            fail = self.numerical_check(costs, ob_control_variations) # 检查数值稳定性，确保权重和控制变化没有出现数值问题。         
            ob_control_variations *= costs
            ob_control_update = np.sum(ob_control_variations, axis=1).reshape(
                (self.num_timesteps, ob_num))
            ob_control_update = ob_control_update.reshape(
                (self.num_timesteps, ob_num))
            ob_control_update = self.savitsky_galoy_ob(ob_control_update, ob_num) 
            if np.isfinite(ob_control_update).all():
                ob_U_update = ob_control_update.reshape(
                    (self.num_timesteps, ob_num))
            self.ob_u = np.zeros((self.ob_num_max),dtype=np.float32)         
            for id in ob_id:
                if id < ob_num:  # 检查索引是否在 ob_num 范围内
                    self.ob_u[id] = ob_U_update[0, id]
    ...
    """@brief: Smooth the resulting control sequence by Savitsky Galoy filter
        使用 Savitzky-Golay 滤波器对控制信号 control_update 进行平滑处理。"""    
    def savitsky_galoy_ob(self, ob_control_update, ob_num):
        new_update = np.zeros_like(ob_control_update)
        for i in range(ob_num): # 对每个控制维度单独应用滤波器。
            new_update[:, i] = scipy.signal.savgol_filter(ob_control_update[:, i], # 
                                                          11,# 滤波器窗口大小
                                                          2, # 边界处理方式为镜像扩展，用于减少边界效应。
                                                          mode='mirror') # 边界处理方式为镜像扩展，用于减少边界效应
        return new_update # 边界处理方式为镜像扩展，用于减少边界效应
    ...
    def compute_control(self, state, cost_params, ob_package ,cost_baseline=None):
        """
        ...
        """
            ...
            min_cost -- 所有轨迹中的最小成本
        """
        #                     障碍：运动信息，数量， 编号；slack标准差，输入，  状态
        # std_n, R, weights, targets, ob, ob_num, ob_id, ob_std_n, ob_U, ob_state = cost_params
        std_n, R, weights, targets= cost_params
        ob, ob_num, ob_id, ob_r, ob_std_n, ob_U, ob_state = ob_package
        # print(f"ob:{ob}")
        # print(f"ob_id:{ob_id}")
        # print(f"dx: {state[3]}")
        # print(f"dy: {state[4]}")  
        ...
        return u, optimal_U, self.draw_sample_U, self.ob_u, normalizer, min_cost
    ...
    def cuda_headers(self):
        ...
        __device__ __constant__ float ob_U_d[{{timesteps}}*{{ob_num_max}}];                           
    ...
    def cuda_rollouts(self):
        ...
        __device__ float get_sla_control_costs(float* u, float* du, float* R, int num)
        {
            // 计算控制成本（控制信号 u 和控制更新 du 的加权成本）。
            // 使用控制代价矩阵 R 和控制信号 u、控制差值 du 计算控制成本。
            int i;
        ...
        __device__ float update_dial_3(float* s, float* ob, int timestep, int timesteps , int ob_num)
        {
            ...
            if(ob_num!=0){
                for (int i = 0; i < ob_num; i++){     
                    ...
                    dist_i = sqrt((s[0]-ob_i[0])*(s[0]-ob_i[0]) + (s[1]-ob_i[1])*(s[1]-ob_i[1]));     
                    ...
                    float d = clamp(dist_i + 0.3*cos_v, {{D}});                                                              
        ...
        __device__ float update_dial_3(float* s, float* ob, int timestep, int timesteps , int ob_num)
        {
            ...
            if(ob_num!=0){
                for (int i = 0; i < ob_num; i++){     
                    ...
                    printf("dx:= %f  ||",dx);  
                    //判断障碍物危险性
                    // cos_ : <0 危险 || =0 刚好掠过 || >0 掠过
                                           
                    //float cos_= dot / dist_i;   
                    dist_i = clamp(dist_i, {{D}});  
                    odia_i = ({{D}}-dist_i)/({{D}} * {{beta_3}}); 
                    //printf("odia_i:= %f  ||",odia_i);                                                                                        
                }                                                                                                                                        
                odia += odia_i;                                                         
            }
            //无障碍物
        ...
        */                                                                                                                                                                                                                                                                   
        // float* ran_sla, float* ob, float* ob_num
        //       松弛变量扰动       障碍信息       障碍数量
        __global__ void rollout_kernel(float* costs_d, float* ran_vec, float* init_state,
                                            float* std_nd, float* R_d, float* weights, float* targets, float* nominal_seq_d,
                                                float* ran_sla, float* init_state_sla, 
                                                    float* r_sla, float* std_n_slad,
                                                        float* ob, float* ob_num, float* ob_id)
        {
            ...
            //初始化 障碍物相关 信息                                      
            //__shared__ float state_sla_shared[{{BLOCK_DIM_X}}*({{ob_num_max}} + {{ob_num_max}})];  //存储每个线程的状态 s 和控制输入 u。                                                   
            //__shared__ float slack_var_shared[{{BLOCK_DIM_X}}*{{ob_num_max}}]; //存储控制扰动 ob_du。 
            //__shared__ float std_n_sla[{{ob_num_max}}]; //存储控制输入的标准差和控制成本矩阵的权重，用于所有线程共享的代价计算。                               
            //__shared__ float R_sla[{{ob_num_max}}];

                // 动态共享内存声明
                // 计算共享内存的总大小                           
            //int max_sharedMemSize = {{BLOCK_DIM_X}} * (2 * {{ob_num_max}} + {{ob_num_max}}) * sizeof(float) + {{ob_num_max}} * sizeof(float);
            __shared__ float sharedMemory[{{BLOCK_DIM_X}} * (2 * {{ob_num_max}} + {{ob_num_max}}) + {{ob_num_max}} + {{ob_num_max}}];
                                            //                                          //
                // 将共享内存划分为不同部分
            float *state_sla_shared = sharedMemory;
            float *slack_var_shared = &sharedMemory[{{BLOCK_DIM_X}} * (2 * {{ob_num_max}})];
            float *std_n_sla        = &sharedMemory[{{BLOCK_DIM_X}} * (2 * {{ob_num_max}}) + {{BLOCK_DIM_X}} * {{ob_num_max}}];                               
            float *R_sla            = &sharedMemory[{{BLOCK_DIM_X}} * (2 * {{ob_num_max}}) + {{BLOCK_DIM_X}} * {{ob_num_max}} + {{ob_num_max}}];
                                                                    
                // 初始化局部状态
            float *s_sla, *u_sla, *du_sla;
            s_sla = &state_sla_shared[tdx * (2 * ob_num_s)]; // s 指向状态变量的起始地址
            u_sla = &state_sla_shared[tdx * (2 * ob_num_s) + ob_num_s]; // u 指向控制变量的起始地址
            du_sla = &slack_var_shared[tdx * ob_num_s]; // du_sla 指向松弛变量扰动的起始地址
...
//            }
            ...
            if(ob_num_s != 0){
                for (int i = tdy; i < ob_num_s; i+= blockDim.y)
                {
                    std_n_sla[i] = std_n_slad[i]; //将 std_nd 控制标准差 和 R_d 控制成本矩阵 加载到共享内存 std_n 和 R 中。
                    R_sla[i] = r_sla[i];                      
                }        
                //printf("std_n_sla[0]:%f",std_n_sla[0]);
                                            
                for (int i = tdy; i < ob_num_s; i+=blockDim.y)
                {
                    s_sla[i] = init_state_sla[i];//加载到状态 s 中。
                }
            }    
            ...
            {   // 开始模拟轨迹的主循环，循环次数为 timesteps。
                ...
                if(ob_num_s != 0){                           
                    compute_policy(s_sla, u_sla, ob_num_s, ob_num_s); //使用 compute_policy 函数从反馈控制器获取初始控制估计 u。
                    }   
                // 更新轨迹退火 指数 
                ...
                //if(global_index == 2495){printf("dial_3:= %f  ||",dial_3);}   
                Covariance = exp(dial_1+dial_2+dial_3); 
                //Covariance = exp(dial_3);                                                       
                //if(Covariance>1.2){printf("Covariance=%f    ||",Covariance);}                           
                // Load std_n, R, and the initial state
                for (int j = tdy; j < {{control_dim}}; j+= blockDim.y)
                {
                    
                    //std_n[j] = clamp(std_nd[j]*Covariance, 0.4);
                    std_n[j] = clamp(Covariance, 0.4); 
                    //std_n[j] = std_nd[j];                                           
                    R[j] = 1 * {{lambda_}} / std_n[j];
                    //std_n_sla[j] = std_n_slad[j];                       
                    std_n_sla[j] = clamp(Covariance, 0.3); //将 std_nd 控制标准差 和 R_d 控制成本矩阵 加载到共享内存 std_n 和 R 中。
                    R_sla[j] = 1 * {{lambda_}} / std_n_sla[j];                                             
                }                     
                ...
                if(ob_num_s != 0){                       
                    // 获取松弛变量控制量 还要改
                    for (int j = tdy; j < ob_num_s; j+=blockDim.y)
                    {
                        u_sla[j] += ob_U_d[i*ob_num_s + j]; // 将当前时间步的控制序列 U_d 添加到控制输入 u。
                        // Noise free rollout
                        if ((tdx == 0 && bdx == 0))
                        {
                            du_sla[j] = 0; //如果是主线程（tdx == 0 && bdx == 0），则 du[j]（控制扰动）设为零，表示无扰动的情况。
                        }
                    else
                        { 
                            //其他线程根据 ran_vec 和标准差 std_n 生成随机扰动 du。
                            du_sla[j] = std_n_sla[j]*ran_sla[(blockDim.x*bdx + tdx)*{{timesteps}}*ob_num_s + i*ob_num_s + j];
                        }
                    }
                    ...
                    for (int j = tdy; j < ob_num_s; j+= blockDim.y)
                    {
                        // 保存每个时间步的控制输入和扰动到 ran_sla 中，便于后续计算。
                        ran_sla[(blockDim.x*bdx + tdx)*{{timesteps}}*ob_num_s + i*ob_num_s + j] = u_sla[j] + du_sla[j];
                    }                                           
                }       
                ...
                if (tdy == 0){
                    ...
                    if(ob_num_s != 0){
                        slack_kinematics(s_sla, u_sla, du_sla, ob_num_s, {{dt}}, i);  
                    }                     
                    //printf("dx=%f           |",s[3]);
                    //printf("dy=%f           |",s[4]);                                              
                }
                ...
                if (tdy == 0)
                {
					...
					running_cost = get_state_cost_with_slack(next_s, s, weights, targets, i, {{timesteps}}, ob, ob_num_s, s_sla);
                    ...
                    if(ob_num_s != 0){                                             
                        slack_cost =  slack_get_state_cost(s_sla, ob_num_s);                                                            
                        //控制 slack变化率代价，目前不加      
                        slack_cost += get_sla_control_costs(u_sla, du_sla, R_sla, ob_num_s);                                                                           
                        cost += slack_cost;
                        }                                        
                }
                __syncthreads();
