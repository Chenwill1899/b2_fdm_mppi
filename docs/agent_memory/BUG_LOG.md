...
| B-012 | open | CUDA RCBF is not yet full soft/slack RCBF | Current omni CUDA controller ports the old relative-velocity barrier modes as a cost penalty, not the old slack-variable RCBF formulation | Keep the implementation labeled as RCBF-style cost; add soft/slack RCBF later only after CUDA omni baseline remains stable. |
