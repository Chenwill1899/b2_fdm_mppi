...
| B-006 | open | Straight-obstacle scene passes goal but gets too close to obstacle | Real MPPI run reached target but min clearance was `0.1954 m`, below `safety_dist=0.3 m`; run printed repeated `clash!!!` warnings | Increase obstacle/CBF safety effect or tune control/path cost while preserving final distance and compute-time criteria. |
